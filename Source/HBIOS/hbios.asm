...
#IF (MEMMGR == MM_Z180)
	...
	BIT 7,A
	JR	Z,HBX_BNKSEL1		; IF NC, WANT ROM PAGE, SKIP AHEAD
	XOR	%00010001		; SET BIT FOR HI 512K, CLR BIT 0
HBX_BNKSEL1:
...
SLICE_MEDSIZ	.FILL 4,0		; CAPACITY OF ACTUAL MEDIA (SECTORS)
...
;
	; GET AND RECORD MEDIA CAPACITY
	LD	B,BF_DIOCAP		; GET CAPACITY
	LD	A,(SLICE_UNIT)		; GET UNIT
	LD	C,A			; PUT IN C
	CALL	DIO_DISPATCH		; DO IT
	RET	NZ			; ABORT ON ERROR
	LD	BC,SLICE_MEDSIZ		; POINT TO MEDSIZ
	CALL	ST32			; SAVE MEDIA CAPACITY
;
...
;
	; DE:HL NOW CONTAINS THE STARTING SECTOR FOR SLICE
	; NOW CONFIRM THAT SLICE FITS WITHIN THE CAPACITY OF THE
	; MEIDA ITSELF
	PUSH    HL			; SAVE THE SECTOR OFFSET (SPS * SLICE NUMBER)
	PUSH    DE
;
...
;
	...
	; NO NEED SAVE ACTUAL RESULT
	OR	A			; CLEAR CARRY FOR SBC
	LD	HL,(SLICE_MEDSIZ+0)	; CAPACITY LSW
	POP	DE			; REQUIRED LSW
	SBC	HL,DE			; CAPACITY - REQUIRED (LSW)
	LD	HL,(SLICE_MEDSIZ+2)	; CAPAITY MSW
	POP	DE			; REQUIRED MSW
	SBC	HL,DE			; CAPACITY - REQUIRED (MSW)
;
	; POP STARTING OFFSET SECTOR
...
;
	; REQUIRE - CAPACITY -> GENERATES BORROW IF CAPITY > REQUIREMENT
	JR      C,EXT_SLICE5C 		; HANDLE IF SLICE DOES NOT FIT
;
EXT_SLICE6Z:
